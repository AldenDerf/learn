...
- The future Postman tutorial will practice GET, POST, PUT, DELETE, request URLs, route and query parameters, JSON request bodies, `Content-Type`, `req.params`, `req.query`, `req.body`, HTTP status codes, response bodies, and response headers. It must use no database. POST, PUT, and DELETE will first show request intent and Express handling: POST does not automatically save data permanently, PUT does not automatically update a database record, and DELETE does not automatically delete one permanently. Database persistence belongs to the later database chapter. The tutorial has not been authored yet.
